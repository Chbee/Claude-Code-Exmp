...
#### 3.1 캐시 고도화
- [x] 3.1.1 캐시 상태 확인 로직 (searchDate 비교)
- [x] 3.1.2 캐시 메타정보 제공 (fetchedAt 절대 시간)
- [x] 3.1.3 actor 기반 동시 읽기/쓰기 보호

#### 3.2 네트워크 상태 감지
- [x] 3.2.1 네트워크 모니터 구현 (NWPathMonitor, @Sendable)
- [x] 3.2.2 오프라인 상태 State
- [ ] 3.2.3 온→오프라인 전환 시 배너 + Toast(info) 알림 *(Phase E 결정: 환율 영역 인라인 캐시 시각 표기로 대체)*

#### 3.3 오프라인 UI 피드백
- [x] 3.3.1 Toolbar 소형 인디케이터
- [ ] 3.3.2 환율 영역 위 오프라인 배너 (절대 시간 병기) *(Phase E 결정: 인라인 상대 시각 표기로 대체)*
...
- [x] 4.1.1 숫자 입력 테스트
- [x] 4.1.2 사칙연산 테스트
- [x] 4.1.3 소수점 테스트 (0. 자동 완성 포함)
- [x] 4.1.4 AC/C/백스페이스 테스트 (C: display만 리셋 확인)
- [x] 4.1.5 엣지 케이스 테스트 (0 나누기, 연산자 교체, = 반복, = 후 연산자, 연산자 후 =, 음수 결과)
- [x] 4.1.6 10자리 제한 테스트 (정수부 기준, 초과 시 숫자만 차단)

#### 4.2 환율 변환 테스트
- [x] 4.2.1 통화 변환 테스트 (Decimal 정밀도, fractionDigits)
- [x] 4.2.2 소수점 처리 테스트
