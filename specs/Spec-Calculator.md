...
| 0으로 나누기 | Toast(warning, "0으로 나눌 수 없어요") + 상태 유지. 연산자 변경으로 복구 가능 |
| 소수점 중복 입력 | 두 번째 `.` 무시 |
| 음수 결과 | display에 `-2` 등 표시 허용. 환율 변환 결과는 **0 처리**. 양수→음수로 전이될 때 1회 `Toast(warning, title="음수 결과", message="환율 변환 결과는 0으로 표시돼요")` 발화 (음수 상태 지속 중 재발화 없음). rateDisplay(`"1 USD = …"`)는 영향 없이 그대로 표시. |
| 정수부 10자리 초과 상태에서 입력 | 숫자/소수점 추가 입력만 차단. 연산자, 백스페이스, AC/C는 정상 동작 |
| `=` 결과가 정수부 **12자리** 초과 | `Toast(error, "계산 결과가 너무 커요")` + 이전 display 유지. 근거: 여행 도메인 현실 최대치 — 입력 10자리 ÷ 0.01(최소 나눗수) = 12자리. 13자리 이상은 일상 곱셈 시나리오로 도달 불가 → 키패드 오입력 신호로 간주하여 차단. 정의: `CalculatorReducer.maxResultIntegerDigits` |
