...
- [~] 위치 기반 자동 설정으로 JP/TH/VN/PH/CN 국가 시 해당 통화 매핑 (EU는 fallback Toast 정상) ← QA 이월 (CoreLocation reverse geocoding 통합 검증, 매핑 로직 자체는 `CurrencyTests.swift` line 40–57에서 커버 완료)
